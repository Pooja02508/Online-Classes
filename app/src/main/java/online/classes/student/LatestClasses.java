...
public class LatestClasses extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_latest_classes);
        ...
        firebaseFirestore.collectionGroup("Subject")
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    ...
                    for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                        ...
                        // Parse startedDate to Date object
                        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM", Locale.getDefault());
//                        SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault());
                        Date startedDate;
                        try {
                            startedDate = formatter.parse(startedDateString);
                        } catch (ParseException e) {
                            // Handle parsing exception
                            continue; // Skip this document if parsing fails
                        }

                        // Check if the class started on or after the current date
                        if (startedDate != null && !startedDate.before(currentDate)) {
                            // Construct the class details string
